...
def verificar_placeholders_esquecidos(caminho, conteudo):
    padroes = ["COLA_AQUI", "TODO", "FIXME", "XXX_PLACEHOLDER"]
    for padrao in padroes:
        if padrao in conteudo and padrao != "COLA_AQUI_O_DSN_DA_SENTRY":
            # Sentry DSN vazio é aceitável (opcional); qualquer outro placeholder não devia ficar
            ocorrencias = [l for l in conteudo.split(padrao) ]
            if padrao == "COLA_AQUI":
                # Ignora especificamente o DSN opcional da Sentry
                ...
                AVISOS.append(f"{caminho}: contém marcador '{padrao}' por resolver")
